...
void addBits(struct bitfile * file, int code, int length) {
  for(int i = length - 1; i >= 0; i--) {
     bitfile_write_bit(file, (code >> i) % 2);
  }
}
...
void huffcoder_encode(struct huffcoder * this, char * input_filename, char * output_filename) {
  ...
  struct bitfile * output_file = bitfile_open(output_filename, "w");
  unsigned char c = fgetc(input_file);
  while(!feof(input_file)) {
    addBits(output_file, this->codes[c], this->code_lengths[c]);
    c = fgetc(input_file);
  }

  addBits(output_file, this->codes[4], this->code_lengths[4]);
...
void huffcoder_decode(struct huffcoder * this, char * input_filename, char * output_filename) {
  ...
  while(bitfile_end_of_file(input_file) == 0) {
    struct huffchar * p = this->tree;
    while(p->is_compound) {
      if(bitfile_read_bit(input_file)) p = p->u.compound.right;
      else p = p->u.compound.left;
    }
    unsigned char character = p->u.c;
    if(character == 4) input_file->is_EOF = 1;
    if(bitfile_end_of_file(input_file) == 0) fputc(character, output_file);
  }
